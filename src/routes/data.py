...
async def upload_data(request: Request, project_id: str, file: UploadFile, app_settings: Settings = Depends(get_settings)):
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST, 
            content={
                "message": f"{ResponseSignal.FILE_UPLOAD_FAILED.value} Error: {str(e)}"}
            )
...
async def process_endpoint(request: Request, project_id: str, process_request: ProcessRequest):
    ...
    if not project_files_ids or len(project_files_ids) == 0:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST, 
            content={"message": ResponseSignal.NO_FILES_ERROR.value}
        )
    ...
    for asset_id, file_id in project_files_ids.items():
        ...
        # Insert into Database safely using your ChunkModel
        inserted_count = await chunk_model.insert_many_chunks(chunks=file_chunks_records)
        no_files+=1

    # Return the exact response you requested
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={
            ...
            "chunks": [
                ...
                for idx, chunk in enumerate(file_chunks[:3]) # Return only the first 3 chunks for preview
            ]
        }
    )
